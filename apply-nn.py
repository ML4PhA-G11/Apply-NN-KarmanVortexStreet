...
def parse_args():
    ...
    p.add_argument("--animate", action="store_true", default=False,
                   help="Produce a GIF of the NN-predicted velocity field")
    p.add_argument("--gif-fps", type=int, default=5,
                   help="Frames per second for the GIF animation")
    p.add_argument("--update-steps", type=int, default=25,
...
def make_animation(model, args):
    ...
    ux[obstacle] = 0.0;  uy[obstacle] = 0.0
    ux[:, 0]     = 0.0;  uy[:, 0]     = 0.0
    ux[:, -1]    = 0.0;  uy[:, -1]    = 0.0
    ...
    for step in range(1, n_steps + 1):
        ...
        rho_out = 1.0
        iy = slice(1, -1)
        ux_out = (-1.0 + (f[-1, iy, 0] + f[-1, iy, 2] + f[-1, iy, 4]
                          + 2.0*(f[-1, iy, 1] + f[-1, iy, 5] + f[-1, iy, 8])) / rho_out)
    ...
    def update(i):
        ...
        ax.imshow(speed.T, origin='lower', cmap='jet',
                  vmin=0, vmax=U_max, aspect='auto', extent=[0, Nx, 0, Ny])
        ux_i = frames_ux[i].copy();  ux_i[obstacle] = 0.0
        uy_i = frames_uy[i].copy();  uy_i[obstacle] = 0.0
...
def main():
    ...
    model = keras.models.load_model(args.model_path, custom_objects={
        'rmsre': rmsre,
